...
*
*ABAP views are:
*- ZSCV2_DdicTable     = Database tables
*- ZSCV2_DdicView      = DDic views
*- ZSCV2_DdicView      = DDic CDS views -> must be changed
*- ZSCV2_EntityCdsView = Entity CDS views

*- ZSCV2_AbapView      = All the above views
*- ZSCV2_CdsView       = DDic CDS views + Entity CDS views
...
CLASS ZSCV2_view_search_dp DEFINITION.
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ts_ddic_view,
        ...
        AbapViewName                  TYPE ZSCV2_AbapViewByView-AbapViewName,
    ...
    METHODS get_views_by_source_names
      ...
      RETURNING VALUE(view_list) TYPE ZSCV2_view_search_dp=>tt_ddic_view_list
      RAISING   zcx_generic_exc.

    TYPES t_view_rng TYPE RANGE OF ZSCV2_AbapViewByView-AbapViewName.
...
CLASS ZSCV2_view_search_dp IMPLEMENTATION.
  ...
  METHOD get_views_by_source_names.
    ...
    DATA db_table_view_list TYPE ZSCV2_view_search_dp=>tt_ddic_view_list.
...
*      SELECT
*        FROM ZSCV2_AbapViewAliasField AS Field
*        FIELDS
*          Field~AbapViewName,
*          Field~FieldName,
  ...
  METHOD _get_abap_views_by_ddl.
    ...
    TYPES:
      BEGIN OF ty_temp_parent_view,
        ParentAbapViewName  TYPE ZSCV2_AbapViewParent-ParentAbapViewName,
        ParentAbapViewType  TYPE ZSCV2_AbapViewParent-ParentAbapViewType,
        ParentDdlSourceName TYPE ZSCV2_AbapViewParent-ParentDdlSourceName,
        ParentDdicViewName  TYPE ZSCV2_AbapViewParent-ParentDdicViewName,
    ...
    DATA child_view_rng TYPE RANGE OF ZSCV2_AbapViewParent-ChildAbapViewName.
    ...
    WHILE 1 = 1.
      ...
      SELECT
        FROM ZSCV2_AbapViewParent( p_DdicCdsBasedOnDllResourceInd = 'X' )
        FIELDS
          ParentAbapViewName,
          ParentAbapViewType,
          ParentDdlSourceName,
...
CLASS ZSCV2_main_ctl DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    DATA view_list TYPE ZSCV2_view_search_dp=>tt_ddic_view_list.
...
CLASS ZSCV2_main_ctl IMPLEMENTATION.
        ...
        CASE sy-ucomm.
          ...
          WHEN OTHERS.
            ...
            IF s_dbtab[] IS NOT INITIAL.
              ...
              DATA(view_search_dp) = NEW ZSCV2_view_search_dp( ).
...
*              s_domnm[] IS NOT INITIAL.
*
*              view_search_dp = NEW ZSCV2_view_search_dp( ).
...
"Application
"------------------------------------------------
DATA main_controller TYPE REF TO ZSCV2_main_ctl.
