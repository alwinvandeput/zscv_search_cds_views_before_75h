...
define view entity ZSCV2_AbapViewByView
  with parameters
    p_AbapViewName                 :vibastab,
    p_DdicCdsBasedOnDllResourceInd :abap_boolean

  as select from ZSCV2_AbapViewByViewGroupBy(
                 p_AbapViewName                  : $parameters.p_AbapViewName,
                 p_DdicCdsBasedOnDllResourceInd  : $parameters.p_DdicCdsBasedOnDllResourceInd
                 ) as AbapViewHier

  association [1..1] to ZSCV2_AbapView as _AbapView        on  _AbapView.AbapViewName = $projection.AbapViewName

  association [0..1] to ZSCV2_CdsView  as _CdsView         on  _CdsView.DdlSourceName = $projection.DdlSourceName
